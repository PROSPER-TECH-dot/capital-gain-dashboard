...
DROP POLICY IF EXISTS "Anyone authenticated can read own or admin all transactions" ON public.transactions;

CREATE POLICY "Read own or admin all transactions"
...
CREATE POLICY "Read own or admin all investments"
...
CREATE POLICY "Authenticated can read gift codes"
ON public.gift_codes FOR SELECT TO authenticated USING (true);

CREATE POLICY "Admins can insert gift codes"
...
ON public.gift_code_redemptions FOR INSERT TO authenticated
WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Read own or admin all roles"
