...
import { Users, ChevronRight, Copy, Check, Link2 } from 'lucide-react';
...
const MyTeamPage = () => {
  ...
  // Build referral tree (simplified with localStorage data)
  const getDirectReferrals = (code: string) => users.filter(u => u.upline?.username === users.find(x => x.referralCode === code)?.username);
  const l1 = users.filter(u => u.upline?.username === user.username);
  const l2 = l1.flatMap(u => users.filter(x => x.upline?.username === u.username));
  const l3 = l2.flatMap(u => users.filter(x => x.upline?.username === u.username));
      ...
      <div className="px-4 -mt-6 space-y-4">
                  ...
                  levelUsers.map(u => (
                    <div key={u.id} className="glass rounded-xl p-3 flex items-center justify-between">
                      ...
                      <p className="text-xs font-semibold text-primary">
                        +{((u.cumulativeIncome || 0) * earning / 100).toLocaleString()} UGX
                      </p>
                    </div>
                  ))
                )}
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input readOnly value={user.referralCode} className="flex-1 px-3 py-2.5 rounded-xl glass text-sm font-bold text-foreground tracking-widest" />
              <button onClick={() => copyToClipboard(user.referralCode, 'code')} className="btn-copy text-xs flex items-center gap-1">
