...
const Auth = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const ok = login(loginId, loginPass);
    if (ok) navigate('/home');
    else setError('Invalid credentials or account banned');
  };
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const ok = register({ username: regUsername, email: regEmail, phone: regPhone, password: regPass, referralCode: regReferral || undefined });
    if (ok) navigate('/home');
    ...
    <div className="min-h-screen flex items-center justify-center p-4 gradient-hero">
      <div className="w-full max-w-md animate-scale-in">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold font-heading text-primary-foreground">{settings.websiteName}</h1>
        ...
        <div className="glass rounded-2xl p-6">
          ...
          {isLogin ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button type="submit" className="w-full btn-accent py-3 text-sm">
                Login
              </button>
            </form>
          ) : (
